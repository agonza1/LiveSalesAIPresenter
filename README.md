...
- Render a HeyGen streaming video avatar when `HEYGEN_API_KEY` and `HEYGEN_AVATAR_ID` are configured.
...
flowchart LR
  ...
  Web -->|voice start / WebRTC offer| Pipecat[Pipecat voice service]
  ...
  Pipecat -->|audio response + transcript events| Web
```
...
- **Pipecat (`apps/pipecat`)**: live voice orchestration, WebRTC handshake, OpenAI Realtime integration, and tool execution against FastAPI.
...
8. Use text Q&A, simulated voice questions, or live voice when Pipecat/OpenAI Realtime is configured.
...
- The active finish line is the proven voice/session loop plus a HeyGen video avatar surface; the old GPT realtime orb visualization has been replaced by streamed avatar video.
